...
function run() {
    ...
    function computeLineSegments() {
      ...
      for (var offset = 0; offset > -period; offset--) {
        ...
        for (
          ...
          theta < thetamax;
          theta += getdtheta(theta, linespacing, hrySpiralR, hrySpiralF)
        ) {
          thetaold = theta >= thetamin ? theta : thetamin;
          thetanew =
            theta + getdtheta(theta, linelength, hrySpiralR, hrySpiralF);
  ...
  function getPointByAngle(theta, hrySpiralF, angleoffset, hrySpiralR) {
    ...
    point.alpha =
      Math.atan((((y * hrySpiralF) / hrySpiralR) * 0.1 + 0.02 - z) * 40) *
        0.35 +
      0.65;
  ...
  function getdtheta(theta, lineLength, hrySpiralR, hrySpiralF) {
    return (
      lineLength /
      Math.sqrt(
        hrySpiralR * hrySpiralR + hrySpiralF * hrySpiralF * theta * theta
      )
    );
  }
